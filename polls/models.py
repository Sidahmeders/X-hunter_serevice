from django.db import models

# Create your models here.

class Question(models.Model):
    question_text = models.CharField(max_length=150, default="what is")
    pub_date = models.DateTimeField(default="date published")

    # def __str__(self):
    #     return self.question_text
        

class Choice(models.Model):
    question = models.ForeignKey(Question, default=0, on_delete=models.CASCADE)
    choice_text = models.CharField(max_length=150, default="life or death")
    votes = models.IntegerField(default=0)

    # def __str__(self):
    #     return self.choice_text


class Boxer(models.Model):
    skill = models.CharField(max_length=100, default="gear second")
    weight = models.IntegerField(default=60)

    # def __str__(self):
    #     return self.skill

